...
async function buildSiteSnapshot(): Promise<SiteSnapshot> {
  ...
  const data = await client.fetch<{
    ...
    "categoryCount": count(*[_type == "categoryPage"])
  }`);
  ...
  const estimatedSitemapEntries =
    5 +
    data.posts.length +
    data.landingPageCount +
    data.categoryCount;

  return {
    totalPosts: data.posts.length,
    totalLandingPages: data.landingPageCount,
...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.log(
      `[SEO Audit] Snapshot: ${snapshot.totalPosts} posts, ${snapshot.totalLandingPages} landing pages, ${snapshot.totalCategories} categories`
    );
  } catch (err) {
  ...
  if (resendApiKey) {
    try {
      ...
      const fromEmail =
        ...
        "How To Work Leads <noreply@howtoworkleads.com>";
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#1f2937;max-width:900px;margin:0 auto;padding:20px;">
  ...
  <div style="background:linear-gradient(135deg,#0f172a,#1e40af);color:white;padding:28px 32px;border-radius:12px;margin-bottom:24px;">
    <table style="width:100%;" cellpadding="0" cellspacing="0">
      <tr>
        <td>
          <h1 style="margin:0 0 4px;font-size:24px;">SEO Audit Report</h1>
          <p style="margin:0;opacity:0.85;font-size:14px;">How To Work Leads — ${report.runDate.split("T")[0]}</p>
        </td>
  ...
  <p style="color:#9ca3af;font-size:12px;">Generated by the How To Work Leads SEO audit cron job. Full backlog saved to data/seo-backlog.json. Audit details saved to data/seo-audits/${report.runDate.split("T")[0]}.json.</p>
</body></html>`;
}
