...
export async function sendWeeklyReport(
  ...
  const fromEmail =
    ...
    "HowToWorkLeads <noreply@howtoworkleads.com>";
  ...
  const subject = `[${status}] HowToWorkLeads Weekly Content — ${report.weekStartDate}`;
...
function buildReportEmail(report: WeeklyReport): string {
  ...
  const imageHtml =
    ...
    <ul style="line-height: 1.5; padding-left: 20px;">
      ${created.map((o) => `<li>${escape(o.title)} <span style="color: #6b7280; font-size: 13px;">— photo by ${escape(o.photographer || "Unsplash")}</span></li>`).join("")}
      ${issues.map((o) => `<li style="color: ${issueColor};">${escape(o.title)} <span style="font-size: 13px;">— ${escape(o.status)}${o.detail ? `: ${escape(o.detail)}` : ""}</span></li>`).join("")}
    </ul>`;
        })()
      : "";
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; color: #1f2937; max-width: 800px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: linear-gradient(135deg, ${color}, #1B4D3E); color: white; padding: 24px 32px; border-radius: 12px; margin-bottom: 24px;">
    ...
    <p style="margin: 0; opacity: 0.9;">HowToWorkLeads — Week of ${report.weekStartDate}</p>
  ...
  <p style="color: #9ca3af; font-size: 12px;">Generated by HowToWorkLeads.com weekly content cron.</p>
</body>
</html>`;
}
