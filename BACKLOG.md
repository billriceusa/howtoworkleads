...
- [~] **P1 — Deepen the IUL cluster around the pos-9.7 winner.** **2026-07-03 update (`/brsg-session`):** verify-built pass found the 4 IUL posts were *already* fully up-linked (each links to `buy-iul-leads`, `aged-iul-leads`, the `how-to-work-iul-leads` playbook, the ROI calculator, and the ALS CTA) — so the specced up-linking was already done. The real gap was **sibling cross-linking** (the 4 posts didn't reference each other). Built a "Related IUL lead guides" block into each of the 4 posts linking the other three (blogPost has no relatedPages field + the blog template has no related-module, so these are in-body links). **Staged as 4 drafts pending publish** (autonomy 2): `drafts.dufjwBP1dSaXDJiWEvyvzk` (iul-leads-cost), `drafts.7mhvXgalLI1Z9dHGbBXAEU`, `drafts.OIsioHNif9uqwjpNmqQITF`, `drafts.OIsioHNif9uqwjpNmqQHRE`. Publish in Studio → Blog Posts to complete. *Traffic + conversion.* **M.**
